...
Для генерации теста для функции go в 1 из пакетов можно использовать консольную утилиту:
```bash
python console_util.py {package_path} {func_name}
```

- `package_path` - путь до пакета
- `func_name` - имя функции

Сгенерированная функция будет сохранена в файл `{func_name}_test.go`.

При необходимости дообученная модель автоматически скачается с huggingface.
